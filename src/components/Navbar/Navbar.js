...
function Navbar({ user, episodeFilm, film, episodeID }) {
  ...
  const getComment = async () => {
    ...
    try {
      const res = await (
        await axios.get(
          "https://backend-test-production-0b5f.up.railway.app/api/comment?filmID=" + film.filmID + "&episodeID=" + episodeID
        )
      ).data;
      setComments(res);
      setIsLoading(true);
  ...
  const handleSendComment = async (comment) => {
    if (user.userId) {
      const req = await axios.post("https://backend-test-production-0b5f.up.railway.app/api/comment", {
        userID: user.userId,
